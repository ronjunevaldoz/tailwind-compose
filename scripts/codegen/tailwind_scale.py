"""Shared token data for tailwind-compose codegen scripts.

Single source of truth for the Tailwind default spacing scale and color
palette. Values are taken from the public Tailwind CSS documentation
(1rem == 16dp equivalence for spacing).
"""

# (kotlin-safe suffix, dp value)
SPACING_SCALE = [
    ("0", 0),
    ("px", 1),
    ("0_5", 2),
...
GENERATED_HEADER = (
    ...
    "// Regenerate with: python3 scripts/codegen/{script}\n"
)


def scale_suffix(key: str) -> str:
    """Turn a scale key into a valid Kotlin identifier suffix (PascalCase for
    non-numeric tokens; underscores replaced with "p" for fractional steps,
    e.g. "0_5" -> "0p5", since ktlint's function-naming rule rejects
    underscores in function names)."""
    if key[0].isdigit():
        return key.replace("_", "p")
